...
* <U>feature selection(특성 선택)과 feature extraction(특성 추출)의 공통점</U>
    * 데이터 특성의 수를 줄이기 위해 사용되는 기술임 **(차원 축소)**. 
  
* <U>feature selection과 feature extraction의 차이점</U>
    * feature selection: sequential back-ward selection(SBS)와 같이 알고리즘을 사용 시 기존의 특성 중에 특성을 선택함. (Chapter 4의 후반부 내용)
    * feature extraction: 특성에서 얻은 정보를 토대로하여 데이터를 새로운 특성 공간으로 변환(결합,투영)함. 
* feature extraction
    * 차원 축소의 맥락에서 feature extraction은 관련 정보를 유지하는 것을 목표로 데이터 압축에 대한 접근 방식
...
* 이 섹션에서는 비지도 선형 변환 기술인 **주성분 분석(principal component analysis ;PCA)** 에 대해 논의하며, feature extraction 및 dimensionality reduction에 대해 논의함.

* **주성분 분석(principal component analysis ;PCA)**
    * PCA는 특성 간 상관 관계를 기반으로 데이터 패턴을 식별하는 데 도움을 줌. 
    * PCA는 고차원 데이터에서 최대 분산의 방향을 찾는 것을 목표로 하며, 새로운 부분 공간에 데이터를 원래의 데이터 또는 더 작은 차원으로 투영함. 
...
*  이번 섹션에서는, PCA의 4가지 단계를 다루고 파이썬으로 구현함.
    step 1. 데이터 표준화
    step 2. 공분산 행렬 구성 
    step 3. 공분산 행렬의 고유값 및 고유 벡터 추출 
    step 4. 고유 벡터 순위 카운트를 위한 고윳값 정렬
...
* **(데이터 표준화 전처리 과정임)** 위 코드를 통해 와인 데이터 셋을 읽어들인 후, train과 test 데이터 세트로(각각 70%, 30%)나누어 단위 분산으로 표준화 함.
* 다음 step으로 공분산 행렬(2개의 확률변수의 상관관계를 나타냄.)을 구성함. 
...
* 고유 벡터와 고유값을 수동으로 계산한는 것은 복잡하기 때문에, 와인 데이터셋에서 covariances 행렬의 고유 쌍을 얻기위해 **Numpy**의 **linalg.eig** 함수를 사용함.
